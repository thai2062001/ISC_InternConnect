...
function SchoolManager() {
  ...
  useEffect(() => {
    const addresss = accounts.map((city) => city.address);
    const uniqueLocations = [...new Set(addresss)];
  ...
  useEffect(() => {
    if (address === 'all') {
      ...
      setFilteredAccounts(accounts.filter(dt => dt.location === address));
    }
  }, [address, accounts]);
  ...
  return (
    <div className="App">
      ...
      <div className={cx('wrapper')}>
        <h1 align="center">Trang quản lý sinh viên</h1>
